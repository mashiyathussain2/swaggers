...
type Collection interface {
	...
	GetCatalogsBySubCollection(id primitive.ObjectID) (*schema.GetCatalogsBySubCollectionResp, error)
...
func (ci *CollectionImpl) GetCatalogsBySubCollection(id primitive.ObjectID) (*schema.GetCatalogsBySubCollectionResp, error) {

	ctx := context.TODO()

	findQuery := bson.M{
		"sub_collections._id": id,
	...
	var catalogResp *schema.GetCatalogsBySubCollectionResp

	err := ci.DB.Collection(model.CollectionColl).FindOne(ctx, findQuery, opts).Decode(&catalogResp)

	if err != nil {
		return nil, errors.Wrap(err, "error finding the collection")
	}
